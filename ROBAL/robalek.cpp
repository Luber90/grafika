...
Robal::Robal(std::vector<glm::vec4> vert, std::vector<glm::vec4> norm, std::vector<glm::vec2> uv, float* color, Camera* kam) : Model(vert, norm, uv, color) {
	kier = glm::vec3(0, 0, 1);
	kamera = kam;
}

void Robal::draw(ShaderProgram* sp, glm::mat4 P, glm::mat4 V) {
	sp->use();
